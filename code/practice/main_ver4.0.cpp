...
/*
函数功能：服务器信息结构体的排序规则，将服务器按照性CPU+MEM升序排列 
input:
	a: 服务器a的结构体
	b: 服务器b的结构体
output:
...
vector<Request> Request_process(vector<string> request_list, vector<VMState> VM_list){
	...
	for(i=0;i<request_list.size();i++){
		...
		if(request.type.compare("add")){
			request.vmID = atoi(request_split[1].substr(1).c_str());
			...
			request.vmID = atoi(request_split[2].substr(1).c_str());
		}
	...
	for(int i=0;i<Requests.size();i++){
		...
		if(!Requests[i].type.compare("add")){
			...
			if(VMs[Requests[i].vmName].node_flag == 0){
				if(VMs[name].cpu>cpu_max){
					cpu_max = VMs[name].cpu;
				}
				if(VMs[name].mem>mem_max){
					mem_max = VMs[name].mem;
				}
			}
		}
	}
//	cout << "BEFORE PURCHASE:" <<cpu_max <<"|||"<<mem_max<<endl; 
	// 计算系统中剩余的CPU和内存总数
	...
	// 计算需要的CPU和内存总数 
	long long cpu_demand = 7*cpu_request-cpu_rest;
	long long mem_demand = 7*mem_request-mem_rest;
	// 找到空间不那么小的第一台服务器 
	int pos;
	for(pos=0;pos<Hosts.size();pos++){
		if(Hosts[pos].cpu >cpu_max*5 && Hosts[pos].mem >mem_max*5){
			break;
		}
	}
	// 根据计算结果进行购买 
	//  
	if(cpu_rest>=4*cpu_request && mem_rest>=4*mem_request){
	}
	else{
		int num1 = ceil(cpu_demand*1.0 / Hosts[pos].cpu);
		int num2 = ceil(mem_demand*1.0 / Hosts[pos].mem);
		int num_1 = max(num1,num2);
		int num3 = ceil(cpu_request*1.0 / Hosts[pos].cpu);
		int num4 = ceil(mem_request*1.0 / Hosts[pos].mem);
...
int vm_allocate(int cpu, int mem, int vmID, string vmName, int node_flag, vector<HostState> &Host_list, vector<VMState> &VM_list){
	//	cout <<"CPU AND MEM OF THE NODE:" <<  cpu <<"|||"<<mem << endl; 
	int temp=0;
...
int allocate(vector<Request> Requests, vector<HostState> &Host_list, vector<VMState> &VM_list, unordered_map<string, VM> VMs){
	...
	for(int i=0;i<Requests.size();i++){  
		...
		if(!Requests[i].type.compare("add")){
			...
			int temp = vm_allocate(cpu, mem, vmID, vmName, node_flag, Host_list, VM_list);
			if(temp==1){
				value = 1;
				return value;
			}
...
//	cout << VMs["vm6U79X"].vmName << "|||" << VMs["vm6U79X"].cpu << "|||" << VMs["vm6U79X"].mem << "|||" << VMs["vm6U79X"].node_flag << endl; 
	 //从输入数据中依次提取日请求，并进行处理
	int hostnum = atoi(input_data[0].c_str()); 
	int vmnum = atoi(input_data[hostnum+1].c_str());
...
//				cout << Requests[j].type << "|||" << Requests[j].vmName << endl;
//			}
//		}
//		for(int j=0;j<Requests.size();j++){
//			cout << Requests[j].type << "|||" << Requests[j].vmName << endl;
...
//		}
		...
		int temp = allocate(Requests, Host_list, VM_list, VMs);
		if(temp==1){
			break;
		}
