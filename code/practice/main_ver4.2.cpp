...
//	cout << "BEFORE PURCHASE:" <<cpu_max <<"|||"<<mem_max<<endl; 
	...
	// 计算需要的CPU和内存总数 
	long long cpu_demand = 10*cpu_request-cpu_rest;
	long long mem_demand = 10*mem_request-mem_rest;
	// 找到空间不那么小的第一台服务器 
	int pos;
	for(pos=0;pos<Hosts.size();pos++){
		if(Hosts[pos].cpu>cpu_max*3 && Hosts[pos].mem >mem_max*3){
			break;
		}
	}
	// 根据计算结果进行购买 
	//  
	if(cpu_rest>=10*cpu_request && mem_rest>=10*mem_request){
	}
	else{
